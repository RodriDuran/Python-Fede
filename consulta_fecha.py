...
try:
    ...
    with con.cursor() as cur:
        ...
        cur.execute('select * from consumo where date >='+str(timestamp_i)+' and date <='+str(timestamp_f)+ ';')
        version = cur.fetchall()
        archivo="consulta.csv"
        csvfile=open(archivo,'w', newline='')
        obj=csv.writer(csvfile)
        obj.writerow(['id','date','valor'])
        cantidad_lineas=0
        ...
        cur.execute('select * from vivienda where date >='+str(timestamp_i)+' and date <='+str(timestamp_f)+ ';')
        version = cur.fetchall()
        archivo="consulta-vivienda.csv"
        csvfile=open(archivo,'w', newline='')
        obj=csv.writer(csvfile)
        obj.writerow(['id','date','temperatura','humedad'])
        cantidad_lineas=0
